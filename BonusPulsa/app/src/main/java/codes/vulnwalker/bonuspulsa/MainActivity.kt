...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    var idPopupAds : String = "ca-app-pub-3417587124775040/8440775920"
    ...
    fun requestKlik(){
        ...
        val postRequest = object : StringRequest(Method.POST, URL.CEK_IKLAN, Response.Listener<String>{
            ...
            override fun getParams(): Map<String, String> {
                val params = HashMap<String, String>()
                params.put("email", publicEmail)
                params.put("jenis_iklan", "KLIK IKLAN")
                return params
            }
        }
        postRequest.retryPolicy = DefaultRetryPolicy(0, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT)
        ...
        mRewardedVideoAd.rewardedVideoAdListener=object : RewardedVideoAdListener {
            ...
            override fun onRewardedVideoAdClosed() {
                ...
                if(videoTerklik.equals("1")){
                    ...
                    val rewardPoint = (20..65).random()
                    Toast.makeText(baseContext, "Anda Mendapatkan "+rewardPoint.toString()+" point", Toast.LENGTH_SHORT).show()
                    getReward(rewardPoint,"TONTON VIDEO")
                }else{
                    val pesanResponse = DialogInterface.OnClickListener { dialog, which ->
                        when (which) {
                            DialogInterface.BUTTON_POSITIVE -> {
...
//                }
    ...
    private fun getReward(hadiah : Int, jenisIklan : String) {
        val queue = Volley.newRequestQueue(this@MainActivity)
        val response: String? = null
        var currentSaldo = "0"
        val postRequest = object : StringRequest(Method.POST, URL.GET_SALDO, Response.Listener<String>{
            ...
            if(err.equals("")){
                ...
                val jumlah = hadiah + currentSaldo.toInt()
                tambahSaldoLocal(jumlah)
                tambahSaldoServer(jumlah,jenisIklan,hadiah)
                refreshBalance()
...
//        spinner.visibility = View.GONE
    ...
    fun absenHarian(){
        ...
        val postRequest = object : StringRequest(Method.POST, URL.ABSEN, Response.Listener<String>{
            ...
            val err = resp.getString("err")
            if(err.toString().equals("")){
                val rewardPoint = (100..500).random()
                getReward(rewardPoint,"ABSEN HARIAN")
                val pesanResponse = DialogInterface.OnClickListener { dialog, which ->
                    when (which) {
                        DialogInterface.BUTTON_POSITIVE -> {
                        }
                ...
                val builder = AlertDialog.Builder(this)
                builder.setMessage("Terimakasih telah melakuka absen, point bertambah " + rewardPoint.toString()).setPositiveButton("Tutup", pesanResponse)
                        .setNegativeButton("", pesanResponse).show()
